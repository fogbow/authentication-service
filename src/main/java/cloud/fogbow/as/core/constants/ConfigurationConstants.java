package cloud.fogbow.as.core.constants;

public class ConfigurationConstants {
    // Service configuration
    public static final String LOCAL_MEMBER_ID_KEY = "xmpp_jid";
    public static final String HTTP_REQUEST_TIMEOUT_KEY = "http_request_timeout";
    public static final String BUILD_NUMBER_KEY = "build_number";

    // Plugins
    public static final String TOKEN_GENERATOR_PLUGIN_CLASS_KEY = "token_generator_plugin_class";

    // Token Generator Plugins configurations
    public static final String CLOUDSTACK_ENDPOINT_KEY = "cloudstack_endpoint";
    public static final String LDAP_BASE_KEY = "ldap_base";
    public static final String LDAP_ENDPOINT_KEY = "ldap_endpoint";
    public static final String LDAP_ENCRYPT_TYPE_KEY = "ldap_encrypt_type";
    public static final String OPENNEBULA_ENDPOINT_KEY = "opennebula_endpoint";
    public static final String OPENSTACK_KEYSTONE_V3_ENDPOINT_KEY = "openstack_keystone_v3_endpoint";
    public static final String SHIB_PUBLIC_FILE_PATH_KEY = "shib_public_key_file_path";
}
